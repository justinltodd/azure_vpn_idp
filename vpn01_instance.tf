...
data "template_file" "vpn01_server_configuration_file" {
  ...
  vars = {
    ...
    VPN_HOST           = "${var.bluevpn01_hostname}"
    ...
    INTERNAL_DOMAIN    = "${var.BLUEVPN01_INTERNAL_DOMAIN}"
  }
}

# Template for openvpn server LDAP PAM Auth configuration file ./templates/server.conf.template
data "template_file" "vpn01_serverLDAP_configuration_file" {
...
data "template_file" "vpn01_client_template_file" {
...
data "template_file" "vpn01_clientLDAPAuth_template_file" {
...
data "template_file" "vpn01_lighttpd_template_file" {
  template = "${file("./templates/lighttpd_centos.template")}"

  vars = {
    HOST     = "${var.bluevpn01_hostname}"
    LOCATION = "${azurerm_resource_group.vpn_hub_vnet-rg.location}"
    ADMIN    = "useradmin"
    PASS     = "CHANGEME1234"
  }
...
data "template_file" "vpn01_openvpn_pam_template_file" {
  template = "${file("./templates/openvpn.pam.template")}"

  vars = {
    GROUP = "${var.BLUEVPN01_GROUP}"
  }
}

# Generate random text for a unique storage account name
resource "random_id" "bluevpn01_randomId" {
  keepers = {
...
resource "azurerm_storage_account" "bluevpn01_storage" {
  name                     = "diag${random_id.bluevpn01_randomId.hex}"
  ...
  tags = {
    environment = "Server: ${var.bluevpn01_hostname}"
  }
}

# Create virtual machine
resource "azurerm_virtual_machine" "bluevpn01" {
  name                  = "${var.bluevpn01_hostname}"
  location              = "${azurerm_resource_group.vpn_hub_vnet-rg.location}"
  resource_group_name   = "${azurerm_resource_group.vpn_hub_vnet-rg.name}"
  network_interface_ids = ["${azurerm_network_interface.bluevpn01_nic.id}"]
  ...
  storage_os_disk {
    name              = "${var.bluevpn01_hostname}_os"
    caching           = "ReadWrite"
  ...
  os_profile {
    computer_name  = "${var.bluevpn01_hostname}"
  ...
  boot_diagnostics {
    enabled     = "true"
    storage_uri = "${azurerm_storage_account.bluevpn01_storage.primary_blob_endpoint}"
  }
  ...
  provisioner "remote-exec" {
    ...
    connection {
      host        = "${azurerm_public_ip.bluevpn01Pubeth01.ip_address}"
      type        = "ssh"
      user        = "useradmin"
  ...
  provisioner "file" {
    content     = "${data.template_file.vpn01_lighttpd_template_file.rendered}"
  ...
  provisioner "file" {
    content     = "${data.template_file.vpn01_server_configuration_file.rendered}"
  ...
  provisioner "file" {
    content     = "${data.template_file.vpn01_serverLDAP_configuration_file.rendered}"
  ...
  provisioner "file" {
    content     = "${data.template_file.vpn01_client_template_file.rendered}"
  ...
  provisioner "file" {
    content     = "${data.template_file.vpn01_clientLDAPAuth_template_file.rendered}"
  ...
  provisioner "file" {
    content     = "${data.template_file.vpn01_openvpn_pam_template_file.rendered}"
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "firewall-cmd --permanent --direct --passthrough ipv4 -t nat -A POSTROUTING -s 10.8.0.0/24 -o ${azurerm_network_interface.bluevpn01_nic.private_ip_address} -j MASQUERADE",
      "firewall-cmd --reload",
      ...
      "hostnamectl set-hostname ${var.bluevpn01_hostname}.example-intra.com --static",
      "echo '${azurerm_network_interface.bluevpn01_nic.private_ip_address}   ${var.bluevpn01_hostname}.example-intra.com' | cat - /etc/hosts | sudo tee /etc/hosts",
      ...
      "./easyrsa --batch --req-cn=${var.bluevpn01_hostname}-RootCA build-ca nopass",
      "./easyrsa build-server-full  ${var.bluevpn01_hostname} nopass",
      "./easyrsa gen-crl",
      "cp pki/ca.crt pki/private/ca.key pki/issued/${var.bluevpn01_hostname}.crt pki/private/${var.bluevpn01_hostname}.key /etc/openvpn/easy-rsa/pki/crl.pem /etc/openvpn/server/",
      ...
      "chmod 700 /etc/openvpn/server/${var.bluevpn01_hostname}.key",
      "chmod 755 /etc/openvpn/server/ta.key",
      "chmod g+s /etc/openvpn/clients/",
      "chmod g+s /etc/openvpn/easy-rsa/",
      "certbot certonly --standalone -n -d ${var.bluevpn01_hostname}.centralus.cloudapp.azure.com --email noreply@example.com --agree-tos --redirect --hsts",
      "cat /etc/letsencrypt/live/${var.bluevpn01_hostname}.${azurerm_resource_group.vpn_hub_vnet-rg.location}.cloudapp.azure.com/privkey.pem /etc/letsencrypt/live/${var.bluevpn01_hostname}.${azurerm_resource_group.vpn_hub_vnet-rg.location}.cloudapp.azure.com/cert.pem > /etc/letsencrypt/live/${var.bluevpn01_hostname}.${azurerm_resource_group.vpn_hub_vnet-rg.location}.cloudapp.azure.com/combined.pem",
...
resource "azurerm_public_ip" "bluevpn01Pubeth01" {
  name                = "${var.bluevpn01_hostname}-public"
  resource_group_name = "${azurerm_resource_group.vpn_hub_vnet-rg.name}"
  location            = "${azurerm_resource_group.vpn_hub_vnet-rg.location}"
  allocation_method   = "Static"
  domain_name_label   = "${var.bluevpn01_hostname}" #//adds dns using hostname.centralus.cloudapp.azure.com
...
# Network Interface
resource "azurerm_network_interface" "bluevpn01_nic" {
  name                      = "${var.bluevpn01_hostname}-eth0"
  location                  = "${azurerm_resource_group.vpn_hub_vnet-rg.location}"
  resource_group_name       = "${azurerm_resource_group.vpn_hub_vnet-rg.name}"
  network_security_group_id = "${azurerm_network_security_group.bluevpn01-sg.id}"
  ...
  ip_configuration {
    ...
    public_ip_address_id          = "${azurerm_public_ip.bluevpn01Pubeth01.id}"
  }
