...
resource "random_id" "bluemgmt01_randomId" {
  keepers = {
...
resource "azurerm_storage_account" "bluemgmt01_storage" {
  name                     = "diag${random_id.bluemgmt01_randomId.hex}"
  ...
  tags = {
    environment = "Server: bluemgmt01 admin portal"
  }
}

# Create virtual machine
resource "azurerm_virtual_machine" "bluemgmt01" {
  name                  = "bluemgmt01"
  location              = "${azurerm_resource_group.vpn_hub_vnet-rg.location}"
  resource_group_name   = "${azurerm_resource_group.vpn_hub_vnet-rg.name}"
  network_interface_ids = ["${azurerm_network_interface.bluemgmt01_nic.id}"]
  ...
  boot_diagnostics {
    enabled     = "true"
    storage_uri = "${azurerm_storage_account.bluemgmt01_storage.primary_blob_endpoint}"
  }
  ...
  provisioner "remote-exec" {
    ...
    connection {
      host        = "${azurerm_public_ip.bluemgmt01Pubeth01.ip_address}"
      type        = "ssh"
      user        = "useradmin"
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "dnf -y update",
      "hostnamectl set-hostname bluemgmt01.example-intra.com --static",
      "echo '${azurerm_network_interface.bluemgmt01_nic.private_ip_address}   bluemgmt01.example-intra.com' | cat - /etc/hosts | sudo tee /etc/hosts",
      "dnf clean packages",
      "echo 'nameserver 9.9.9.9' >> /etc/resolv.conf",
...
resource "azurerm_public_ip" "bluemgmt01Pubeth01" {
  name                = "bluemgmt01-public"
  resource_group_name = "${azurerm_resource_group.vpn_hub_vnet-rg.name}"
  location            = "${azurerm_resource_group.vpn_hub_vnet-rg.location}"
  allocation_method   = "Static"
  domain_name_label   = "bluemgmt01" #//adds dns using hostname.centralus.cloudapp.azure.com
...
# Network Interface
resource "azurerm_network_interface" "bluemgmt01_nic" {
  name                      = "bluemgmt01-eth0"
  location                  = "${azurerm_resource_group.vpn_hub_vnet-rg.location}"
  resource_group_name       = "${azurerm_resource_group.vpn_hub_vnet-rg.name}"
  network_security_group_id = "${azurerm_network_security_group.bluemgmt01-sg.id}"
  ...
  ip_configuration {
    ...
    public_ip_address_id          = "${azurerm_public_ip.bluemgmt01Pubeth01.id}"
  }
